...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order_code:
        try:
            ...
            # تحقق إن الـ update شغل فعلاً (RLS ممكن يمنعه بصمت)
            if result.data:
                logging.info(f"[SUPABASE] ✅ Updated order {order_code} → chat_id={chat_id}, lang={lang}")
            else:
                logging.warning(f"[SUPABASE] ⚠️ Update returned no rows for order_code={order_code} — تحقق من RLS أو إن الكود صحيح")
