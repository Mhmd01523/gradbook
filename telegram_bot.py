...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tg_lang  = user.language_code or "en"          # Telegram's detected language
    lang     = map_tg_lang(tg_lang)                # map to our supported set

    # Extract order_code from deep-link payload (/start ORDER_CODE)
    order_code = " ".join(context.args) if context.args else ""

    logging.info(f"Start: chat_id={chat_id}, tg_lang={tg_lang}, mapped={lang}, order={order_code}")

    # Save chat_id + bot_lang to Supabase orders table
    if order_code:
        try:
            ...
            logging.warning(f"Supabase update failed: {e}")
